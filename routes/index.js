...
module.exports = (app, db) => {
	...
	app.get('/lookup/:lookup', (req, res) => { 
		var lookup = req.params.lookup ? req.params.lookup : 'shudder',
			options = {
				url: `${process.env.GUIDEBOX_API}/movies/all/0/10/${lookup}`,
	...
	app.get('/search/:term', (req, res) => {
		var searchterm = encodeURIComponent(req.params.term),
			options = {
				url: `${process.env.GUIDEBOX_API}/search/movie/title/${searchterm}`
			};
